...
class WuerstchenDiffNeXt(ModelMixin, ConfigMixin):
    ...
    def _down_encode(self, x, r_embed, effnet, clip=None, block_indices=None, scales=None):
        level_outputs = []
        
        if block_indices is not None:
            down_idx = block_indices[0]
            down_scale = scales[0]
        
        
        #print('length of down blocks:', self.down_blocks.__len__())
        for i, down_block in enumerate(self.down_blocks):
            # print('down block i:', i)
            # print('down_block len:', down_block.__len__())
            effnet_c = None
    ...
    def _up_decode(self, level_outputs, r_embed, effnet, clip=None, block_indices=None, scales=None):
        
        if block_indices is not None:
            skip_idx = block_indices[1]
            skip_scale = scales[1]
            
            up_idx = block_indices[2]
            up_scale = scales[2]
        ...
        for i, up_block in enumerate(self.up_blocks):
            ...
            for j, block in enumerate(up_block):
                if isinstance(block, ResBlockStageB):
                    ...
                    if block_indices is not None and (i == skip_idx):
    ...
    def forward(self, x, r, effnet, clip=None, x_cat=None, eps=1e-3, return_noise=True, block_indices=None, scales=None):
        ...
        x_in = x
        x = self.embedding(x)
        level_outputs = self._down_encode(x, r_embed, effnet, clip, block_indices=block_indices, scales=scales)
        x = self._up_decode(level_outputs, r_embed, effnet, clip, block_indices=block_indices, scales=scales)
