...
class ScoreSdeVeScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_pred(
        ...
        prev_sigma = self.discrete_sigmas[0].to(sample.device) #
        coefficient = adjacent_sigma/sigma #
        #print('sigma:', sigma)
        #print('prev_sigma:', prev_sigma)
        constant = sigma * ((sigma**2 - prev_sigma**2) ** 0.5) #
        ...
        prev_sample = prev_sample_mean + coefficient * diffusion * noise  # add impact of diffusion field g

        if not return_dict:
            return (prev_sample, prev_sample_mean)

        predicted = sample + constant * model_output
        #residual = prev_sample - predicted
        
        return SdeVeOutput(prev_sample=prev_sample, prev_sample_mean=prev_sample_mean), predicted, noise
    ...
    def step_correct(
        ...
        return SchedulerOutput(prev_sample=prev_sample), noise
