...
class UNet2DConditionModel(
    ...
    def __init__(
        ...
        # with open('sdxl_images/info.txt', 'w') as f:
        #     f.write(f'down block types: {down_block_types} \n')
        #     f.write(f'mid block type: {mid_block_type} \n')
        #     f.write(f'up block types: {up_block_types} ')
    ...
    def get_aug_embed(
        ...
        aug_emb = None
        #print('addition_embed_type:', self.config.addition_embed_type)
    ...
    def forward(
        ...
        fmap_pth: str = None,
        time_idx: int = None,
        channel_optim: bool = False,
        channel_weights: torch.nn.Parameter = None,
        scales: list = None,
        ...
        #print('aug_emb.shape:', aug_emb.shape)
        ...
        def save_fmap(fmap, save_pth, skip_sample=None, skip_save_pth=None, time_idx=None, fourier_pth=None, abs=False):
            ...
            if skip_save_pth is not None:
                os.makedirs(skip_save_pth, exist_ok=True)

            fmap_img = fmap.detach().cpu()
            
            #print('fmap_img shape:', fmap_img.shape)
            ...
            #     frequency, freqs = fourier_save(fmap_img[0])
            
            #     plt.plot(frequency, freqs, label='backbone')
            #     plt.xlabel('frequency')
            #     plt.ylabel('log amplitude')

            #     if skip_sample is not None:
            #         frequency, freqs = fourier_save(skip_img[0])
            
            #         plt.plot(frequency, freqs, label='skip')
            #         plt.xlabel('frequency')
            #         plt.ylabel('log amplitude')
            ...
            if fourier_pth:
                ...
                plt.tight_layout()
                os.makedirs(fourier_pth, exist_ok=True)
                plt.savefig(os.path.join(fourier_pth, f't_{time_idx+1}.png'))
                plt.close()
            
            
            # norm_fmap_img = torch.cat([norm_fmap_img[0], norm_fmap_img[1]], dim=1)
            
            plt.imshow(fmap_img[0], cmap='coolwarm')
            plt.tight_layout()
            plt.axis('off')
            ...
            if skip_sample is not None:
                # skip_img = torch.cat([skip_img[0], skip_img[1]], dim=1)

                plt.imshow(skip_img[0], cmap='coolwarm')
                plt.tight_layout()
                plt.axis('off')
        ...
        def channel_plot(fmap, save_pth, time_idx, mask_pth, channels=None):
            ...
            mask = mask[0, 0, :, :]
            # sort channels by fmap values in the mask region
        ...
        def cal_ious(fmap : torch.Tensor, masks : dict, save_pth, time_idx, threshold_r=0.7):
            ...
            for key, mask in masks.items():
                ...
                plt.bar(range(iou.shape[1]), iou[0])
                ...
                for k in range(5):
                    channel_idx = top_k_channels[0, k]
                    iou_value = top_k_values[0, k]

                    channel_pth = os.path.join(iou_pth, f'top5/t_{time_idx+1}/')
                    os.makedirs(channel_pth, exist_ok=True)

                    channel_img = fmap[0, channel_idx]
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            if fmap_pth:
                save_pth = os.path.join(fmap_pth, f'down/block_{i}')
                fourier_pth = os.path.join(fmap_pth, f'down/block_{i}/fourier/')
        ...
        if fmap_pth:
            save_pth = os.path.join(fmap_pth, f'down/block_3')
            fourier_pth = os.path.join(fmap_pth, f'down/block_3/fourier/')
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            #print('up block ids:', up_block_ids)
            if up_indices:
                if i in up_indices:
                    if mask is not None:
                        print('mask applied')
                        ...
                        if repeat_encoder:
                            ...
                            sample[0][mask_.squeeze(0).expand_as(sample[0]) == 1] *= up_scale
                            sample[2][mask_.squeeze(0).expand_as(sample[0]) == 1] *= up_scale
                            #sample[[0,2]][mask_.expand_as(sample[[0,2]]) == 1] *= up_scale
                            #sample[0][mask_.expand_as(sample[0]) == 1] *= up_scale
                            
                            #print('in uent difference:', torch.mean(torch.abs(a - sample[0])))

                        else:
                            sample[mask_.expand_as(sample) == 1] *= up_scale

                        #print('in unet difference:', torch.mean(torch.abs(sample[0] - sample[1])))
                        #print('in unet sample 0 ptr:', sample[0].data_ptr())
                        #print('in unet sample 1 ptr:', sample[1].data_ptr())
                    else:
            ...
            #     res_samples[i][mask[None, None, :, :].expand_as(res_samples[i]) == 1] *= skip_scale

            if skip_indices:
                if i in skip_indices:
                    for j in range(len(res_samples)):
                        if mask is not None:
                            mask_ = F.interpolate(mask[None, None, :, :], size=res_samples[j].shape[-2:], mode='nearest')
                            if repeat_encoder:
                                print('repeat encoder')
                                res_samples[j][0][mask_.squeeze(0).expand_as(res_samples[j][0]) == 1] *= skip_scale
                                res_samples[j][2][mask_.squeeze(0).expand_as(res_samples[j][2]) == 1] *= skip_scale
                            else:
                                res_samples[j][mask_.expand_as(res_samples[j]) == 1] *= skip_scale
                        else:
                            if repeat_encoder:
                                print('repeat encoder')
                                res_samples[j][0] *= skip_scale
                                res_samples[j][2] *= skip_scale
                            else:
                                res_samples[j] *= skip_scale


            # print('i:', i)
            # print('sample shape:', sample.shape)

            if channel_optim and i == 2:
                sample = sample * channel_weights

            if fmap_pth:
                backbone_save_pth = os.path.join(fmap_pth, f'up/block_{i}/backbone')
                
                backbone_fourier_pth = os.path.join(fmap_pth, f'up/block_{i}/fourier/')
                skip_save_pth = os.path.join(fmap_pth, f'up/block_{i}/skip')

                s_sample_viz = res_samples[-1]
                save_fmap(sample, backbone_save_pth, skip_sample=s_sample_viz, skip_save_pth=skip_save_pth, time_idx=time_idx, fourier_pth=backbone_fourier_pth)
                ...
                for quad in range(4):
                    channel_quad_backbone_pth = os.path.join(fmap_pth, f'up/block_{i}/channel_quad_{quad}/backbone/')
                    channel_quad_skip_pth = os.path.join(fmap_pth, f'up/block_{i}/channel_quad_{quad}/skip/')
                ...
                #skip_fourier_pth = os.path.join(fmap_pth, f'up/block_{i}/skip/fourier/')

                #save_fmap(s_scale * res_samples[0], skip_save_pth, time_idx=time_idx, fourier_pth=skip_fourier_pth)
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                #print(f'upsample id {i} has a cross attention') idx: 0, 1
            else:
            ...
            #print('upsample shape:', sample.shape)
                #print(f'upsample id {i} dones not have a cross attention') idx: 2

        if fmap_pth:
            backbone_save_pth = os.path.join(fmap_pth, f'up/block_3/backbone')
            
            backbone_fourier_pth = os.path.join(fmap_pth, f'up/block_3/backbone/fourier')
            
            save_fmap(sample, backbone_save_pth, time_idx=time_idx, fourier_pth=backbone_fourier_pth)
            
            # skip_save_pth = os.path.join(fmap_pth, f'up/block_3/skip/t_{time_idx+1}.png')
            
            # skip_fourier_pth = os.path.join(fmap_pth, f'up/block_3/skip/fourier/')

            # save_fmap(s_scale * res_samples[-1], skip_save_pth, time_idx=time_idx, fourier_pth=skip_fourier_pth)
