...
def get_up_block(
    ...
    elif up_block_type == "UpDecoderBlock2D": # this block is used as a defulat setting
...
class CrossAttnUpBlock2D(nn.Module):
    def __init__(
        ...
        #print('cross attn up block 2d num_layers:', num_layers) 
...
class UpBlock2D(nn.Module):
    def __init__(
        ...
        #print('up block 2d num_layers:', num_layers)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for resnet in self.resnets:
            ...
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
            pre_skip_list.append(res_hidden_states.abs().mean(dim=(1, 2, 3)))
            backbone_list.append(hidden_states.abs().mean(dim=(1, 2, 3)))
            
            hidden_states = torch.cat([hidden_states, res_hidden_states], dim=1)
            fusion_list.append(hidden_states.abs().mean(dim=(1, 2, 3)))
            hidden_states = resnet(hidden_states, temb)

        #fusion_list.append(hidden_states.abs().mean().item())
        ...
        # pre_skip = torch.tensor(pre_skip_list)
        # backbone = torch.tensor(backbone_list)
        # fusion = torch.tensor(fusion_list)
        
        pre_skip = torch.stack(pre_skip_list, dim=1)
        backbone = torch.stack(backbone_list, dim=1)
        fusion = torch.stack(fusion_list, dim=1)

        return hidden_states, skip_sample, (pre_skip, backbone, fusion)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for resnet in self.resnets:
            ...
            pre_skip_list.append(res_hidden_states.abs().mean(dim=(1, 2, 3)))
            backbone_list.append(hidden_states.abs().mean(dim=(1, 2, 3)))
            #print('pre shape:', res_hidden_states.abs().mean(dim=(1, 2, 3)).shape)
            #print('backbone shape:', hidden_states.abs().mean(dim=(1, 2, 3)).shape)
